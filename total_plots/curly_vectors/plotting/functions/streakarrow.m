function hh=streakarrow(X0,Y0,U,V,np,arrow,thin)
...
if any(diff(dx)>maxDiff) || any(diff(dy)>maxDiff) || ...
    ...
    xi=linspace(minx,maxx,round(n/thin));
    yi=linspace(miny,maxy,round(m/thin));
...
XY=stream2(Xm,Ym,Um,Vm,Xm,Ym, .1);
%np=15;
Vmag=sqrt(Um.^2+Vm.^2);
Vmin=min(Vmag(:)); Vmax=max(Vmag(:));
Vmag=Vmag(:); x0=Xm(:); y0=Ym(:);
...
%     COL=[cmap(vcol,1) cmap(vcol,2) cmap(vcol,3)];
    ...
    if arrow==1&L>1
        ...
        plot([xa1 x2 xa2],[ya1 y2 ya2],'color',COL, 'linewidth', 1.25); hold on
    else
        continue
...
[~,~,xind] = histcounts(x(gind),xi);
[~,~,yind] = histcounts(y(gind),yi);
