...
years.seasons={'fall', 'spring' ,'summer' ,'winter'};
...
m_text(-69.5,37, [num2str(conf.Plot.wind_scale) 'm/s'],'verticalalignment','bottom');
...
h= title(['MAB Wind:  ' conf.season.name{kk} ' from ' num2str(conf.NDBC.years(jj)) ' to ' num2str(conf.NDBC.years(end)-1)] ,'FontWeight','bold','FontSize',14);
...
k = boundary(LonLat(:,1),LonLat(:,2));
hold on;
m_plot(LonLat(k,1),LonLat(k,2),'k','LineWidth',2);


% [cs,h2]=m_contour(TUV.LonLat(:,1),TUV.LonLat(:,2),TUV.U,[0 0]);
% set(h2,'LineWidth',3,'Color','k')
% clabel(cs,h2,'fontsize',6);


timestamp(1,'plot_mean_wind_vectors_MAB_seasons_nazzaro.m')

 %% create the output directory and filename
output_directory = '/Users/roarty/COOL/01_CODAR/MARACOOS/20131211_MAB_Currents/20180702_Wind_Plots/';
