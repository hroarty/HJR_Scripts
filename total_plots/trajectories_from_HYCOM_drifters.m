...
conf.Plot.script_name='trajectories_from_HYCOM_drifters.m';
conf.Data.Institution='Rutgers University';
conf.Data.Contact='Hugh Roarty hroarty@marine.rutgers.edu';
conf.Data.Info='This data file is US Coast Guard drifter data from SLDMB as provided by ASA along with virtual drifter data from HYCOM';
% conf.Save.directory='/Users/hroarty/COOL/01_CODAR/MARACOOS/20150515_Model_Skill_Score/20160329_Drifter_Data_Virtual_HYCOM/';
conf.Save.directory='/Users/hroarty/COOL/01_CODAR/MARACOOS/20160410_CG_Drifter_Experiment/20160606_Drifter_Data_Virtual_HYCOM/';
...
f = 'http://ecowatch.ncddc.noaa.gov/thredds/dodsC/hycom/hycom_reg1_agg/HYCOM_Region_1_Aggregation_best.ncd';
    % https://ecowatch.ncddc.noaa.gov/thredds/dodsC/hycom/hycom_reg1_agg/HYCOM_Region_1_Aggregation_best.ncd
    
dtime=datenum(2016,5,10):3/24:datenum(2016,06,06);
...
dtime=drifter.time(I):hourTimeStep/24:drifter.time(end);
%% only go out 4 days (since data is hourly go out 4*24 =96 index spots) to test the code
% dtime=drifter.time(I):hourTimeStep/24:drifter.time(I+96);
...
%% 24 hours, since the drifter data is hourly 24 index spots is 24 hours
%% 24*2 simulate for 2 days
  days=2;
%% loop to start at I
...
for jj=1:length(loop1)
    ...
    tspan=drifter.time(loop1(jj)):hourTimeStep/24:drifter.time(loop1(jj))+days;
...
save([conf.Save.directory 'USCG_' drifter.name_str '_HYCOM_skill_score_' datestr(min(D.ti),'yyyy_mm_dd') '.mat'], '-struct', 'D');
