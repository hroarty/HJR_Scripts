function mean_vector_plot_fn_04_drifter(TUVcat,p,dtime,drifter)
...
%     ind=find(drifter{jj}.time==dtime);
    
    [Y,I]=min(abs(drifter{jj}.time-dtime));
    
%     m_plot(drifter{jj}.Lon(ind),drifter{jj}.Lat(ind),'*','Color',M(jj,:));
    m_plot(drifter{jj}.Lon(I),drifter{jj}.Lat(I),'*','Color','k','MarkerSize',20);
end
...
print_path='/Volumes/hroarty/codar/BPU/totals/2012_Cat_File/';

timestamp(1,p.HourPlot.script)

print('-dpng',p.HourPlot.resolution,[p.HourPlot.print_path 'TUV_' p.HourPlot.name '_' p.HourPlot.region '_' sd_str2 '.png'])
