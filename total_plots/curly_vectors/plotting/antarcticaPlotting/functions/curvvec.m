...
%create streamlines
XY=stream2(Xm,Ym,Um,Vm,Xm,Ym, [.35 1000]);
...
len=cellfun(@(x)(size(x,1)),XY); % get length of streams
nline=ceil(interp1(linspace(minspd,maxspd,np),(1:np),spd(:))); % interp min and max speeds to length of lines
nline(spd>maxspd)=np;
nline(spd<minspd)=0;

F0=cell(length(XY),1); %created duplicate cell array of streamlines
...
F1=cellfun(@(x,y)(x(end-1:end,:)),F0,'uni',0); % grab end two data lat/lons from each stream
...
% ibeta = ones(length(x2),1) .* 0.155555555555556;
...
h2=cellfun(@(x,y,z)(plot(x,y,'color',z,'linewidth',lw)),...
    xhead(1:thin:end),yhead(1:thin:end),colors(1:thin:end), 'UniformOutput', false);
