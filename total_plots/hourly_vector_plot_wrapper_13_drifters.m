...
conf.Drifter.Name={'43340','43411','43346'};
conf.Drifter.path='/Users/hroarty/COOL/01_CODAR/MARACOOS/20160410_CG_Drifter_Experiment/20160510_Drifter_Data/mat_files_20160510_to_20160706/';

% load the drifter data
for ii=1:length(conf.Drifter.Name)
    drifter{ii}=load([conf.Drifter.path 'USCG_' conf.Drifter.Name{ii} '_2016_05_10.mat']);   
end
...
%% Define some configuration parameters for the current map
conf.start_date=datenum(2016,5,10,22,0,0);
conf.end_date=datenum(2016,6,9,0,0,0);
conf.x_interval=30;
...
conf.HourPlot.axisLims= [-74-15/60 -73-15/60 39+30/60 40+10/60];% NJ shelf
...
conf.HourPlot.print_path ='/Users/hroarty/COOL/01_CODAR/MARACOOS/20160410_CG_Drifter_Experiment/20180206_Hourly_Maps_Drifters/';
...
%     dtime = month_time(jj):1/24:month_time(jj+1);
    ...
    [f]=datenum_to_directory_filename(conf.(s).BaseDir,dtime,conf.(s).FilePrefix,conf.(s).FileSuffix,conf.(s).MonthFlag,conf.OI.no_hour_flag);
...
%     TUV.dayM=nanmeanTotals(TUVcat,2);
    ...
    mean_vector_plot_fn_04_drifter(TUVcat,conf,dtime,drifter);
