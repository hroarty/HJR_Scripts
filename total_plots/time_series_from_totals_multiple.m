...
U=NaN(length(ind),length(total_data));

for i_row=1:length(ind)
    for i_col=1:length(total_data)
        U(i_row,i_col)=total_data(i_col).U(ind(i_row));
    end
end
...
for i_row=1:length(ind)
    for i_col=1:length(total_data)
        V(i_row,i_col)=total_data(i_col).V(ind(i_row));
    end
end
...
title('October 2009 Surface Velocity Time Series Search Radius 20km from OOI Pioneer Mooring Array')
...
timestamp(1,'COOL/01 CODAR/Tidal_Analysis/time_series_from_totals_multiple.m')
