...
switch pfunc
  case {'m_vec','m_vec_same','m_vec_same_color'}
...
if ~isempty(LL)
  switch pfunc
    ...
    case 'm_vec_same'
      ...
      U=real(vector);
      V=imag(vector);
        h = m_vec( 12, LL(:,1), LL(:,2), U, V, 'headlength', 2.5, 'headangle', 45, 'shaftwidth', .5);
%       h = m_vec( 10, LL(:,1), LL(:,2), U, V, mm, varargin{:} );
    case 'm_vec_same_color'
      vector=U+sqrt(-1)*V;
      vector=vector./mm;
      U=real(vector);
      V=imag(vector);
%         h = m_vec( 12, LL(:,1), LL(:,2), U, V, 'headlength', 2.5, 'headangle', 45, 'shaftwidth', .5);
      h = m_vec( 10, LL(:,1), LL(:,2), U, V, 'k', varargin{:} );
    case {'arrow','m_arrow'}
