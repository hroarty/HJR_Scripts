function [ ofn, hdls, p ] = MARC_driver_plot_totals_orig_HJR( dtime, p, varargin )
%-------------------------------------------------
...
[TUV,I]=maskTotals(data.TUV,p.Totals.Mask2,true); % true keeps vectors in box
...
x_ind=1:p.grid_spacing:length(unique_x);
y_ind=1:p.grid_spacing:length(unique_y);
...
% for ii=1:length(NX)
%    % d=dist(NX(ii),NY(ii),data.TUVorig.LonLat(:,1),data.TUVorig.LonLat(:,2));
%    % hjr 2015-05-08
%     d=dist(NX(ii),NY(ii),data.TUV.LonLat(:,1),data.TUV.LonLat(:,2));
%     ind=find(d<0.03);
%     if ~isempty(ind)
%         marcoos_grid_ind(ii)=ind;
%     end
%     
%     clear d
% end
...
%data.TUVsub=subsrefTUV(data.TUVorig,marcoos_grid_ind);
data.TUVsub=subsrefTUV(data.TUV,marcoos_grid_ind);

%% land mask the data
[TUVmask,I]=maskTotals(data.TUVsub,p.Totals.Mask,true); % false removes vectors on land



%% clean the radials removing any vectors above 0.6
[TUVmask,I]=cleanTotals(TUVmask,p.Totals.MaxTotSpeed,p.OI.cleanTotalsVarargin{:});

% [hdls.plotData] = plotData( data.TUV, 'm_vec',dtime, p.HourPlot.VectorScale, ...
%                               p.HourPlot.plotData_xargs{:});
                              
 [hdls.plotData,I] = plotData2( TUVmask, 'm_vec_same', dtime, p.HourPlot.VectorScale, ...
                               p.HourPlot.plotData_xargs{:} );
%%-------------------------------------------------
