...
conf.NDBC.years=2004:2016;

conf.NDBC.t0=datenum(conf.NDBC.years(1),1,1);
conf.NDBC.tN=datenum(conf.NDBC.years(end),12,31,23,0,0);

conf.NDBC.Sites={'acyn4'};

buoyData=load_buoy(conf.NDBC.t0,conf.NDBC.tN,conf.NDBC.Sites{1});
...
water_temp=NaN(length(yr_vec),365);
std_dev=NaN(length(yr_vec),365);
water_temp=NaN(length(yr_vec),365);
...
for ll=1:length(yr_vec)
    ...
    for ii=1:12
        for jj=1:month(ii)
            ...
            ind=time_vec(:,1)==yr_vec(ll) & time_vec(:,2)==ii & time_vec(:,3)==jj;

            water_temp(ll,kk)=nanmean(buoyData.wtmp(ind)*1.8+32);
            N(ll,kk)=sum(ind);
            std_dev(ll,kk)=nanstd(buoyData.wtmp(ind)*1.8+32);
...
for ll=1:length(yr_vec)
    ...
    water_temp_smooth(:,ll)=smooth(water_temp(:,ll),10);
...
for ll=1:length(yr_vec)
    plot(x,y(:,ll),'LineWidth',2,'Color',M(ll,:))
    pause 
end
...
conf.HourPlot.script='plot_water_temp_daily_mean_by_year.m';
conf.Plot.Filename=['Water_Temp_Daily_Mean_' buoy.name '_' num2str(yr_start) '_' num2str(yr_end) '_v3.png'];
