...
switch frequency
    ...
    case 5
        conf.Diag.Sites={'AMAG','MRCH','HEMP'};
%         conf.Diag.Sites={'MRCH'};
         conf.Diag.Region='Central';
        ymax=360;
        ymin=0;
    case 55
        conf.Diag.Sites={'HOOK','LOVE','BRIG','WILD'};
%         conf.Diag.Sites={'MRCH'};
         conf.Diag.Region='Central2';
        ymax=360;
        ymin=0;
    case 6
        conf.Diag.Sites={'ASSA','CEDR','LISL'};
         conf.Diag.Region='South';
        ymax=360;
        ymin=0;
     case 66
        conf.Diag.Sites={'DUCK','HATY','CORE'};
         conf.Diag.Region='South2';
        ymax=360;
        ymin=0;
...
for t = 1:length(conf.Diag.Sites)
    ...
    hold on
    ylim([ymin ymax])
    set(gca,'YTick',y_ticks,'fontsize',6)
    xlim([datenum(t1) datenum(t2)])
    ylabel(conf.Diag.Sites(t),'fontsize',8,'rot',90,'HorizontalAlignment','right')
...
title([ var ' ' year.str(1:4) ' to ' year.str(1:4)],'fontsize',12)

% maximizeSubPlots(plot_handle)

timestamp(1,'/radial_database/plot_MetaData_FromDB_all.m')

%ht = legend (h,site,'Location','Best');

conf.Plot.PrintPath='/Users/hroarty/COOL/01_CODAR/MARACOOS_II/20180201_2017_Reprocessing/20180320_Radial_Inspection/';
