...
%% loop through each time variable (temp, speed, dir)  to load the wind data and generate the plot
for jj=1:4
...
%        end
       ...
       for kk=1:length(conf.season.name)
           ...
           %% if calculating wind direction use this section
            if jj==4
                ...
                if var.mean(kk)<180
                    var.mean(kk)=  var.mean(kk)+360;
                end
                
                var.std(kk)=math2true(rad2deg(circ_std(data)));
                var.N(kk)=sum(data);
            
            %% else calculate other variable means     
            else
...
%    
    ...
    conf.plot.filename = ['MAB_Climatology_Seasonal_' conf.NDBC.standard_name{jj} '_'  num2str(min(time.min)) '_' num2str(max(time.max)) '.png'];
