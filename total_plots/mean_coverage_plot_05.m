...
conf.HourPlot.axisLims=[-77 -73 34 39];
...
% conf.Plot.PrintPath='/Users/hroarty/COOL/01_CODAR/MARACOOS/20131211_MAB_currents/20170307_Monthly_Figures/';
% conf.Plot.PrintPath='/Users/hroarty/COOL/01_CODAR/MARACOOS_II/20170622_Josh_Erick_Figures/';
% conf.Plot.PrintPath='/Users/hroarty/COOL/01_CODAR/MARACOOS_II/20171201_YR1.5_Progress_Report/';
conf.Plot.PrintPath='/Users/hroarty/COOL/01_CODAR/MARACOOS_II/20180201_2017_Reprocessing/20180305_Reprocessing_Analysis/';
...
%conf.OI.BaseDir='/Volumes/hroarty/data/totals/';
% conf.OI.FilePrefix=['tuv_oi_' conf.HourPlot.DomainName '_CAT_'];
...
time.month=datenum(2017,5:6,1);
time.vec=datevec(time.month);

% months=[datenum(2010,8,27) datenum(2010,8,30)];
% months=[datenum(2011,3,16) datenum(2011,3,20)];
months=time.month;

for ii=1:numel(time.month)-1
% for ii=1
    ...
    [f]=datenum_to_directory_filename(conf.(s).BaseDir,dtime,conf.(s).FilePrefix,conf.(s).FileSuffix,conf.(s).MonthFlag,conf.OI.HourFlag);

    %% concatenate the total data
    % f, TUV, error flag, otherflag, mask
%     [TUVcat,goodCount] = catTotalStructs(f,'TUV',true,false,true,conf.HourPlot.Mask);
    [TUVcat,goodCount] = catTotalStructs(f,'TUVoi',true,false,true,conf.HourPlot.Mask);
    jj = false(size(TUVcat));
...
% use this section if the files are already concatenated
%     date_string=sprintf('%d_%02d',time.vec(ii,1),time.vec(ii,2));
%     % this loads the TUV structured array
%     load([conf.OI.BaseDir conf.OI.FilePrefix date_string '.mat']);
%     TUVcat=TUV;
...
%     end
    ...
    titleStr = {sprintf('%s %s Coverage, %d possible hourly maps', ...
                ...
                sprintf('From %s to %s',datestr(TUVcat.TimeStamp(1),'yyyy-mm-dd HH:MM'),...
                datestr(TUVcat.TimeStamp(end),'yyyy-mm-dd HH:MM'))};
    ...
    sd_str=datestr(TUVcat.TimeStamp(1),'yyyymmdd');
    ed_str=datestr(TUVcat.TimeStamp(end),'yyyymmdd');
    print(1,'-dpng','-r200',[conf.Plot.PrintPath conf.HourPlot.DomainName...
         '_total_coverage_' sd_str '_' ed_str '_Below_0.6_uncertainty.png'])
%          '_total_coverage_' sd_str '_' ed_str '.png'])
    ...
    figure(2)
    plot(TUVcat.TimeStamp,good.number)
...
%     ylabel('Percent of Valid Measurements (Relative to Max for Time Period)')
    ...
    print(2,'-dpng','-r200',[conf.Plot.PrintPath conf.HourPlot.DomainName ...
         '_total_coverage_ts2_' sd_str '_' ed_str '_Below_0.6_uncertainty.png'])
%         '_total_coverage_ts2_' sd_str '_' ed_str '.png'])
