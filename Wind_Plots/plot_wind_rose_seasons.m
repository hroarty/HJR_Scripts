...
for kk=1:length(conf.season.name)
    ...
    for ii=1:length(conf.NDBC.Sites)
       ...
       wind_rose(buoyData{ii}.wfdir(ind),buoyData{ii}.wspd(ind),'dtype','meteo','n',12,'di',0:5:30,'ci',[5 10 15 20])
       
       h= title([buoyData{ii}.name ' Wind Rose:  ' conf.season.name{kk} ' from ' num2str(conf.NDBC.years(1)) ' to ' num2str(conf.NDBC.years(end)-1)] ,'FontWeight','bold','FontSize',14);
        ...
        output_filename = [buoyData{ii}.name '_Wind_Rose_'  conf.season.name{kk} '_' num2str(conf.NDBC.years(1)) '_' num2str(conf.NDBC.years(end)-1) '.png'];
