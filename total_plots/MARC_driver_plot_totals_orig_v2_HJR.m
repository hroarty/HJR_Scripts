function [ ofn, hdls, p ] = MARC_driver_plot_totals_orig_v2_HJR( dtime, p, varargin )
%-------------------------------------------------
...
%
% This script interpolates the data onto a rectangular grid
%-------------------------------------------------
if numel(dtime)>1
...
%% interpolate the data onto the new grid, the true is to spatial interp the errors
TUVi=spatialInterpTotals(data.TUVorig,LonLat,true);

%% land mask the data
[TUVmask,I]=maskTotals(TUVi,p.Totals.Mask,false);

%% mask grid points that do not coincide with the national network grid
[TUVmask,I]=maskTotals(TUVmask,p.Totals.Mask2,true);
