function divergence_plot_for_NC_fn(TUV,p)

hold on
    
%% Pcolor the magnitude of the vectors
 mag = sqrt(TUV.U.^2 + TUV.V.^2); % 
%  pcolor(TUV.LON,TUV.LAT,TUV.Divergence);
 pcolor(TUV.LON,TUV.LAT,TUV.Vorticity);
% shading flat
shading interp
...
%     TUV.V(1:p.HourPlot.grid:end,1:p.HourPlot.grid:end), 1, 1, 1);
...
% hdls.xlabel = xlabel( cax, 'm/day', 'fontsize', 14 );
hdls.xlabel = xlabel( cax, 'vorticity/f', 'fontsize', 14 );
...
hdls.title = title( p.HourPlot.TitleString, 'fontsize', p.HourPlot.TitleFontSize,'color','k');
